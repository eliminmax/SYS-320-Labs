...
$FirewallType = Read-Host -Prompt "Please enter Windows or IPTables: "

switch ($FirewallType.ToLower().Trim()) {
    "windows" {
        ((Get-Content -Path .\ips-bad.tmp) | Join-String -Separator "," -DoubleQuote) -Replace "^",`
        ("New-NetFirewallRule -Name ""BlockKnownBadHosts"" -Enabled True -DisplayName ""Block Known Bad Hosts"""+`
        " -Description ""Block IP addresses known to be malicious or compromised"" -Action Block -RemoteAddress ")`
        | Out-File -FilePath ".\Block-KnownBadIPs.ps1"
    }
    "iptables" {
    ...
    Default { 
        Write-Host "Unrecognized option '$FirewallType'."
        Throw
    }
}